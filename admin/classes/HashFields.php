...
class HashFields {
    ...
    public static function create() {
        ...
        $field_type = HashHelper::get_post('field_type', 'sanitize_text_field');
    ...
    public static function destroy() {
        ...
        check_ajax_referer('hashform_ajax', 'nonce');
        $field_id = HashHelper::get_post('field_id', 'absint', 0);
    ...
    public static function include_new_field($field_type, $form_id) {
        ...
        $field_id = HashFields::create_row($field_values);
        if (!$field_id) {
            return false;
        }
        $field = self::get_field_vars($field_id);
        $field_array = self::covert_field_obj_to_array($field);
        $field_obj = HashFields::get_field_class($field_array['type'], $field_array);
    ...
    public static function setup_new_field_vars($type = '', $form_id = '') {
        ...
        $values['field_key'] = HashHelper::get_unique_key('hashform_fields', 'field_key');
    ...
    public static function get_default_field($type) {
        $field_obj = HashFields::get_field_class($type);
    ...
    public static function import_options() {
        ...
        $field_id = HashHelper::get_post('field_id', 'absint');
        $field = self::get_field_vars($field_id);
    ...
    public static function create_row($values, $return = true) {
        ...
        $new_values['field_key'] = HashHelper::get_unique_key('hashform_fields', 'field_key');
    ...
    public static function destroy_row($field_id) {
        ...
        if (!$field)
            return false;
        $wpdb->query($wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'hashform_entry_meta WHERE field_id=%d', $field_id));
        return $wpdb->query($wpdb->prepare('DELETE FROM ' . $wpdb->prefix . 'hashform_fields WHERE id=%d', $field_id));
    }
    ...
    public static function get_field_vars($field_id) {
        if (empty($field_id))
            return null;
        global $wpdb;
        $query = $wpdb->prepare('SELECT * FROM ' . $wpdb->prefix . 'hashform_fields WHERE id=%d', $field_id); // phpcs:ignore WordPress.DB.PreparedSQL.NotPrepared
    ...
    public static function get_error_msg($field, $error) {
        ...
        $max_length = intval(HashFields::get_option($field, 'max'));
        ...
        $msg = HashFields::get_option($field, $error);
    ...
    public static function include_field_class() {
        ...
        include HASH_FORM_PATH . 'admin/classes/fields/FieldType.php';
        foreach ($classes as $class) {
            include HASH_FORM_PATH . 'admin/classes/fields/' . $class . '.php';
        }
    }
