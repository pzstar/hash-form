...
abstract class FieldType {
    ...
    private function container_classes_array() {
        ...
        $container_class[] = 'hashform-field-type-' . $field['type'];
        //$container_class[] = ($field['type'] == 'captcha' && $global_settings->re_type == 'v3') ? 'hf-hidden' : '';
        ...
        if (in_array($field['type'], array('separator', 'image', 'heading', 'paragraph', 'html'))) {
            ...
            $container_class[] = 'hf-field-alignment-' . $field_alignment;
        }
        ...
        if (!in_array($field['type'], array('separator', 'image', 'heading', 'paragraph', 'html'))) {
            ...
            $container_class[] = $field['classes'] ? $field['classes'] : '';
    ...
    private function container_inner_style() {
        $field = $this->get_field();
        $field_max_width = isset($field['field_max_width']) ? $field['field_max_width'] : '';
        $field_max_width_unit = isset($field['field_max_width_unit']) ? $field['field_max_width_unit'] : '%';
        $inline_style = $field_max_width ? ('--hf-width:' . $field_max_width . $field_max_width_unit . ';') : '';
        if ($field['type'] == 'image_select') {
            $image_max_width = isset($field['image_max_width']) ? $field['image_max_width'] : '';
            $image_max_width_unit = isset($field['image_max_width_unit']) ? $field['image_max_width_unit'] : '%';
            $inline_style .= $image_max_width ? '--hf-image-width: ' . $image_max_width . $image_max_width_unit : '';
        }
        return $inline_style;
    }
    ...
    public function load_single_field() {
        ...
        ?>
        <li id="hf-editor-field-id-<?php echo esc_attr($field['id']); ?>" class="<?php echo esc_attr(implode(' ', $classes)); ?>" data-fid="<?php echo esc_attr($field['id']); ?>" data-formid="<?php echo esc_attr('divider' === $field['type'] ? $field['form_select'] : $field['form_id'] ); ?>" data-type="<?php echo esc_attr($field['type']); ?>">

            <div id="hf-editor-field-container-<?php echo esc_attr($field['id']); ?>" class="hf-editor-field-container" style="<?php echo esc_attr($field_max_width ? ('--hf-width:' . $field_max_width . $field_max_width_unit . ';') : ''); ?><?php echo ((isset($image_max_width) && $image_max_width) ? '--hf-image-width: ' . $image_max_width . $image_max_width_unit : ''); ?>">
    ...
    protected function load_single_field_settings() {
        ...
        $field_id = $field['id'];
        $all_field_types = HashFields::field_selection();
        $type_name = $all_field_types[$field_type]['name'];

        include( HASH_FORM_PATH . 'admin/classes/fields/settings.php' );
    }
    ...
    public function show_single_option() {
        ...
        foreach ($field['options'] as $opt_key => $opt) {
            ...
            require( HASH_FORM_PATH . 'admin/classes/fields/single-option.php' );
        }
    }
    ...
    public function sanitize_value(&$value) {
        return HashHelper::sanitize_value('sanitize_text_field', $value);
    }
    ...
    protected function get_new_field_name() {
        ...
        $fields = HashFields::field_selection();
