...
class HashEntry {
    ...
    public static function route() {
        ...
        $action = isset($_GET[$action_key]) ? htmlspecialchars_decode(wp_unslash($_GET[$action_key])) : '';
    ...
    public static function view($id = 0) {
        ...
        include( HASH_FORM_PATH . 'admin/entries/entry-detail.php' );
    }
    ...
    public static function display_entry_list($message = '', $class = 'updated') {
        ...
        <div class="hf-content">
            <div class="hf-entry-list-wrap wrap">
                ...
                $entry_table = new HashEntryListing();
                $entry_status = HashHelper::get_var('status', 'sanitize_title', 'published');
                $entry_table->views();
                ?>
    ...
    public function listing_page_screen_options() {
        ...
        if (!is_object($screen) || $screen->id != $hash_entry_listing_page || (isset($_GET['hashform_action']) && $_GET['hashform_action'] == 'view'))
            return;
    ...
    public static function process_bulk_actions() {
        ...
        $ids = HashHelper::get_var('entry_id', 'sanitize_text_field');
    ...
    public function process_entry() {
        ...
        $form = HashBuilder::get_form_vars($form_id);

        if (!$form)
            return;
        $errors = '';
        $errors = HashValidate::validate(wp_unslash($data));

        if (empty($errors)) {
            $form_settings = $form->settings;
            $entry_id = self::create($data);

            $send_mail = new HashEmail($form, $entry_id);
    ...
    private static function create_entry($values) {
        $new_values = HashValidate::before_insert_entry_in_database($values);
    ...
    private static function set_value_before_save(&$values) {
        $field = HashFields::get_field_vars($values['field_id']);
        if ($field) {
